...
unsigned long Arguments::Get(const char * name, const unsigned long defaultValue)
{
  ...
  if (t == _arguments.end())
     return defaultValue;
  return strtoul(t->second.c_str(), NULL, 10);
}

template<>
long long Arguments::Get(const char * name, const long long defaultValue)
{
  std::map<std::string, std::string>::iterator t = _arguments.find(name);
  if (t == _arguments.end())
     return defaultValue;
  return strtoll(t->second.c_str(), NULL, 10);
}

template<>
long Arguments::Get(const char * name, const long defaultValue)
{
  std::map<std::string, std::string>::iterator t = _arguments.find(name);
  if (t == _arguments.end())
     return defaultValue;
...
template<>
std::string Arguments::Get(const char *name, const std::string defaultValue)
{
  std::map<std::string, std::string>::iterator t = _arguments.find(name);
  if (t == _arguments.end())
     return defaultValue;
